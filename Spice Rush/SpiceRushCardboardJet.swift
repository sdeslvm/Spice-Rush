...
protocol WebLoadable: AnyObject {
    var state: SpiceRushWebStatus { get set }
...
final class SpiceRushWebLoader: NSObject, ObservableObject, WebLoadable, ProgressMonitoring {
    // MARK: - Свойства

  @Published var state: SpiceRushWebStatus = .standby
    ...
    func observeProgression() {
        progressPublisher
            ...
            .sink { [weak self] progress in
                ...
                let newState: SpiceRushWebStatus = progress < 1.0 ? .progressing(progress: progress) : .finished
...
extension SpiceRushWebLoader: WKNavigationDelegate {
...
extension SpiceRushWebLoader {
    ...
    func publish(_ newState: SpiceRushWebStatus) {
...
private extension SpiceRushWebLoader {
    func updateState(_ newState: SpiceRushWebStatus) {
